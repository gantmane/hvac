...
```mermaid
graph TB
    A[Carbon Pricing Policy] --> B[Carbon Tax]
    A --> C[Cap-and-Trade]
    A --> D[Carbon Offsets]

    B --> E[Fixed Price per tCO₂e]
    E --> F[Applied to Fuel Suppliers]
    F --> G[Passed to Energy Consumers]

    C --> H[Emissions Cap Set]
    H --> I[Allowances Issued/Auctioned]
    I --> J[Emitters Buy/Trade Allowances]
    J --> K[Costs Passed to Consumers]

    D --> L[Verified Emission Reductions]
    L --> M[Credit Generation]
    M --> N[Credit Sale/Use for Compliance]

    G --> O[HVAC Operating Costs Increase]
    K --> O

    O --> P[Equipment Selection Impacts]
    O --> Q[Operating Strategy Changes]

    P --> R[High-Efficiency Equipment]
    P --> S[Electrification vs Gas]
    P --> T[Low-GWP Refrigerants]

    Q --> U[Load Reduction]
    Q --> V[Renewable Energy Integration]

    L --> W[HVAC Efficiency Projects]
    L --> X[Refrigerant Leak Reduction]
