...
```mermaid
graph TD
    A[HVAC Mentorship Program] --> B[Formal Programs]
    A --> C[Informal Programs]

    B --> D[One-on-One Pairing]
    B --> E[Group Mentorship]
    B --> F[Rotational Model]

    C --> G[Peer Mentoring]
    C --> H[Reverse Mentoring]
    C --> I[Project-Based Guidance]

    D --> J[6-24 Month Duration]
    D --> K[Documented Progress]

    E --> L[Cohort Learning]
    E --> M[Shared Resources]

    F --> N[Multi-Specialty Exposure]
    F --> O[3-6 Month Rotations]

    G --> P[Skill Exchange]
    H --> Q[Technology Transfer]
    I --> R[On-Job Learning]
```
...
```mermaid
flowchart LR
    A[Apprentice<br/>0-2 Years] --> B[Junior Technician<br/>2-4 Years]
    B --> C[Journeyman<br/>4-8 Years]
    C --> D[Master Technician<br/>8+ Years]
    C --> E[Service Manager]

    A --> F[Mentor: Master Tech]
    B --> G[Mentor: Service Manager]
    C --> H[Mentor: Operations Lead]

    D --> I[Subject Matter Expert]
    D --> J[Technical Trainer]
    E --> K[Branch Manager]
```
