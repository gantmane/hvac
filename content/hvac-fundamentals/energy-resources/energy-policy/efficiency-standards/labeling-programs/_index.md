...
```mermaid
graph TD
    A[Manufacturer Designs Product] --> B[Laboratory Testing per DOE Test Procedures]
    B --> C{Performance Level?}
    C -->|Meets Federal Minimum| D[EnergyGuide Label Required]
    C -->|Exceeds Top 25%| E[ENERGY STAR Eligible]
    C -->|Exceeds ENERGY STAR| F[CEE Tier Eligible]
    D --> G[Submit to AHRI for Certification]
    E --> G
    F --> G
    G --> H[Third-Party Verification Testing]
    H --> I{Ratings Verified?}
    I -->|Pass| J[Product Listed in AHRI Directory]
    I -->|Fail| K[Corrective Action Required]
    K --> B
    J --> L[Apply EnergyGuide Label]
    J --> M[Apply ENERGY STAR Mark if Qualified]
    J --> N[Market as CEE Tier if Applicable]
    L --> O[Product Available for Sale]
    M --> O
    N --> O
    O --> P[Annual Compliance Testing]
    P --> Q{Still Compliant?}
    Q -->|Yes| O
    Q -->|No| R[Remove Certification]
```
