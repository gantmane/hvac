...
```mermaid
graph TD
    A[Exterior Wall Assembly] --> B[Wall Sleeve Installation]
    B --> C[Weatherproofing Membrane]
    B --> D[Structural Opening]
    B --> E[Drainage Slope 1/4 inch]

    C --> F[Exterior Grille]
    C --> G[Sealed Perimeter]

    D --> H[PTAC/PTHP Chassis]
    H --> I[Cooling Coil]
    H --> J[Heating Element/Heat Pump]
    H --> K[Compressor]
    H --> L[Supply Air Fan]

    E --> M[Condensate Drain Pan]
    M --> N[Exterior Discharge]

    F --> O[Outside Air Intake]
    G --> P[Interior Trim Assembly]
