...
```mermaid
graph TD
    A[State Legislature] -->|Enacts RPS Law| B[RPS Targets]
    B --> C[Utility Obligations]
    C --> D{Compliance Mechanisms}
    D --> E[Direct Renewable Generation]
    D --> F[Renewable Energy Credits]
    D --> G[Alternative Compliance Payments]
    E --> H[Grid Renewable Content]
    F --> H
    G --> I[Penalty/Fund]
    H --> J[Building Electricity Supply]
    J --> K[HVAC Operating Costs]
    J --> L[Fuel Switching Economics]
    K --> M[Building Energy Strategy]
    L --> M
    I --> N[Renewable Energy Fund]
    N --> O[Rebate Programs]
    O --> M
```
