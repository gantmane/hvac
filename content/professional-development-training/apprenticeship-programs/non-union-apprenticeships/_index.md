...
seo_description: "Complete guide to non-union HVAC apprenticeships. Learn about ABC, NCCER, HVAC Excellence programs, competency verification, portable credentials, and career paths."
---
...
```mermaid
graph TD
    A[Power Source] -->|Line Voltage| B[Disconnect]
    B -->|L1, L2| C[Contactor]
    C -->|Switched Power| D[Compressor Motor]
    B -->|R to Common| E[24V Transformer]
    E -->|24VAC| F[Thermostat]
    F -->|Y Call| G[Contactor Coil]
    G -->|Energized| C
    H[Indoor Blower] -->|N| I[Neutral]
    B -->|L1| H
...
```mermaid
graph LR
    A[Competency Module] --> B{Written Test}
    B -->|Pass| C{Hands-On Task}
    B -->|Fail| D[Remedial Training]
    D --> B
    C -->|Pass| E{Employer Verification}
    C -->|Fail| D
    E -->|Verified| F[Competency Recorded]
    E -->|Not Met| D
    F --> G[Progress to Next Level]
