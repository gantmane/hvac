...
```mermaid
graph TD
    A[Frozen Fish from IQF/Blast Freezer] --> B{Application Method}
    B --> C[Dip Glazing]
    B --> D[Spray Glazing]

    C --> C1[Immersion in Chilled Water Tank]
    C1 --> C2[Temperature: 0-2°C]
    C2 --> C3[Dwell Time: 2-5 seconds]
    C3 --> E[Drip Station 5-10 seconds]

    D --> D1[Automated Spray Nozzles]
    D1 --> D2[Water Pressure: 2-4 bar]
    D2 --> D3[Droplet Size: 50-200 μm]
    D3 --> E

    E --> F[Flash Freeze Glaze]
    F --> G[Temperature: -30 to -40°C]
    G --> H[Duration: 30-60 seconds]
    H --> I[Packaging]
    I --> J[Cold Storage -18°C or lower]
```
