...
```mermaid
graph TD
    A[Green Tobacco Loaded on Tiers] --> B[Fire Pit Ignition]
    B --> C[Smoke Generation 600-800°F]
    C --> D[Smoke Enters Through Floor Vents]
    D --> E[Natural Convection Rises Through Tiers]
    E --> F[Smoke Contact with Tobacco Leaves]
    F --> G[Temperature: 110-125°F]
    G --> H[RH Control: 60-70%]
    H --> I{Moisture Content Check}
    I -->|>25%| F
    I -->|<25%| J[Increase Temperature to 130-140°F]
    J --> K[Final Drying Phase]
    K --> L{Final MC 15-18%}
    L -->|Not Achieved| K
    L -->|Achieved| M[Fire Extinguished]
    M --> N[Conditioning Period 3-5 Days]
    N --> O[Dark-Fired Tobacco Complete]
