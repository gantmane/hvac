...
```mermaid
graph TD
    A[National Model Codes] --> B[IMC, IBC, IECC, IFGC]
    B --> C[State Adoption Process]
    C --> D[State Mechanical Code]
    D --> E[County Amendments]
    D --> F[Municipal Amendments]
    E --> G[Local AHJ Enforcement]
    F --> G
    G --> H[Permit Issuance]
    G --> I[Plan Review]
    G --> J[Inspections]
    H --> K[Field Compliance]
    I --> K
    J --> K
    K --> L[Certificate of Occupancy]
