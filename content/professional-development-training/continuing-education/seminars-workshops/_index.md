...
```mermaid
graph TD
    A[HVAC Professional] --> B{Credential Type}

    B --> C[PE License]
    B --> D[NATE Certification]
    B --> E[CEM/BEMP]
    B --> F[LEED/WELL AP]

    C --> G[Attend Seminars/Workshops]
    D --> G
    E --> G
    F --> G

    G --> H{PDH/CEU Eligible?}

    H -->|Yes| I[Obtain Certificate]
    H -->|No| J[Supplemental Learning Only]

    I --> K[Document in Records]
    K --> L[Submit for Renewal]
    L --> M[Maintain Certification]

    J --> N[Apply to Career Development]
...
```mermaid
graph LR
    A[Entry Level<br/>Technician] -->|2-3 years| B[Journeyman<br/>Skills]

    B --> C{Career Path}

    C --> D[Service<br/>Specialist]
    C --> E[Installation<br/>Expert]
    C --> F[Controls<br/>Technician]

    D --> G[Advanced Service<br/>Workshops]
    E --> H[Installation<br/>Best Practices]
    F --> I[BAS Programming<br/>Training]

    G --> J[Master Technician<br/>Certification]
    H --> J
    I --> J

    J --> K[Ongoing Seminars<br/>New Technologies]

    K --> L[Industry Leadership<br/>Instructor Role]
