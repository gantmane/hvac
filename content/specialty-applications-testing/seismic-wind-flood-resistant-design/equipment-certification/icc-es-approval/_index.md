...
keywords:
  ...
  - seismic certification HVAC
  - evaluation service reports
  - code compliance HVAC
...
```mermaid
flowchart TD
    A[Product Development] --> B[Pre-Application Consultation]
    B --> C[Formal Application Submission]
    C --> D[Technical Review]
    D --> E{Complete Information?}
    E -->|No| F[Request Additional Data]
    F --> D
    E -->|Yes| G[Testing Requirements Identified]
    G --> H[Laboratory Testing]
    H --> I[Test Report Submission]
    I --> J[Engineering Evaluation]
    J --> K{Meets Criteria?}
    K -->|No| L[Product Modifications]
    L --> H
    K -->|Yes| M[Draft ESR Preparation]
    M --> N[Manufacturer Review]
    N --> O[Final ESR Issuance]
    O --> P[Listing in ICC-ES Directory]
...
```mermaid
flowchart LR
    A[ESR Issued] --> B[Design Reference]
    B --> C[Specification]
    C --> D[Procurement]
    D --> E[Installation per ESR]
    E --> F[Inspection]
    F --> G{Complies with ESR?}
    G -->|Yes| H[Approval]
    G -->|No| I[Correction Required]
    I --> E
