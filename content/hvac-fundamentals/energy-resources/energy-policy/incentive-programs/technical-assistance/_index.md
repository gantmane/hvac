...
```mermaid
graph TD
    A[Facility Contacts Program] --> B{Eligibility Screening}
    B -->|Qualifies| C[Application Submission]
    B -->|Does Not Qualify| D[Referral to Alternative Resources]
    C --> E[Site Assessment Scheduling]
    E --> F[Data Collection & Analysis]
    F --> G{Audit Type}
    G -->|Level 1| H[Walk-Through Report]
    G -->|Level 2| I[Detailed Engineering Analysis]
    G -->|Level 3| J[Investment-Grade Audit]
    H --> K[Findings Presentation]
    I --> K
    J --> K
    K --> L[Implementation Support]
    L --> M{Additional Services}
    M -->|Yes| N[RCx/Training/M&V]
    M -->|No| O[Ongoing Performance Tracking]
    N --> O
    O --> P[Annual Follow-Up]
```
