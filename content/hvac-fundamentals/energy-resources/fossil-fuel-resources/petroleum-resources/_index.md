...
```mermaid
graph TD
    A[Crude Oil Extraction] --> B[Transportation Pipeline/Tanker]
    B --> C[Refinery Reception]
    C --> D[Atmospheric Distillation Tower]
    D --> E[Light Ends: LPG, Gasoline]
    D --> F[Middle Distillates: Kerosene, Diesel]
    D --> G[Heavy Ends: Fuel Oil, Residuals]
    F --> H[Hydrotreating]
    H --> I[No. 1 Heating Oil Kerosene]
    H --> J[No. 2 Heating Oil Diesel]
    G --> K[Vacuum Distillation]
    K --> L[No. 4 & No. 6 Heavy Fuel Oil]
    I --> M[Wholesale Terminal]
    J --> M
    L --> M
    M --> N[Truck Transport]
    N --> O[Retail Dealer]
    O --> P[End User: Residential/Commercial HVAC]
```
