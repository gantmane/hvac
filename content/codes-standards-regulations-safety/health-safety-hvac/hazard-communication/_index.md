...
```mermaid
graph TB
    A[Hazard Communication Program] --> B[Chemical Inventory]
    A --> C[Labeling System]
    A --> D[Safety Data Sheets]
    A --> E[Employee Training]

    C --> F[GHS Pictograms]
    C --> G[Signal Words]
    C --> H[Hazard Statements]
    C --> I[Precautionary Statements]

    D --> J[16-Section SDS Format]
    J --> K[Identification]
    J --> L[Hazards]
    J --> M[Composition]
    J --> N[First Aid]
    J --> O[Firefighting]
    J --> P[Accidental Release]

    E --> Q[Initial Training]
    E --> R[New Chemical Training]
    E --> S[Annual Refresher]
