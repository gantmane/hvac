...
```mermaid
graph TD
    A[Pre-Application Assessment] --> B[Equipment Selection]
    B --> C[Contractor Certification Check]
    C --> D[Submit Application]
    D --> E{Pre-Approval Required?}
    E -->|Yes| F[Await Approval 2-4 weeks]
    E -->|No| G[Proceed with Installation]
    F --> G
    G --> H[Installation by Licensed Contractor]
    H --> I[Inspection and Documentation]
    I --> J[Submit Final Documentation]
    J --> K{Performance Verification?}
    K -->|Yes| L[Third-Party M&V]
    K -->|No| M[Administrative Review]
    L --> M
    M --> N[Incentive Payment 4-8 weeks]
```
