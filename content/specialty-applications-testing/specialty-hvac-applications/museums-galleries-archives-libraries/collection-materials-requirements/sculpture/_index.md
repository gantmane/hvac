...
```mermaid
graph TB
    subgraph Gallery["Gallery Environmental Zones"]
        A[Gallery Ambient<br/>19-21°C, 50% RH] --> B[Bronze Zone<br/>19-20°C, 35% RH]
        A --> C[Wood Zone<br/>19-21°C, 50% RH]
        A --> D[Stone Zone<br/>18-25°C, 45-55% RH]
        A --> E[Mixed Media Zone<br/>19-20°C, 45% RH]
    end

    subgraph Microclimate["Microclimate Control"]
        B --> F[Sealed Case + Silica Gel]
        E --> G[Active Case Conditioning]
    end

    subgraph Lighting["Lighting Heat Management"]
        H[LED Spotlights] --> I[Surface ΔT < 2°C]
        J[Halogen Spots] -.->|"Avoid"| K[Surface ΔT > 8°C]
    end

    subgraph AirQuality["Air Quality Systems"]
        L[MERV 13 Filtration] --> M[Particulate Control]
        N[Activated Carbon] --> O[SO₂, H₂S Removal]
        O --> B
    end
