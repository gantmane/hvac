...
```mermaid
graph TD
    A[Product Submission] --> B[Application Review]
    B --> C[Testing Protocol Development]
    C --> D[Laboratory Testing]
    D --> E{Testing Results}
    E -->|Pass| F[Engineering Review]
    E -->|Fail| G[Design Modification]
    G --> D
    F --> H[Product Listing]
    H --> I[Follow-Up Services]
    I --> J[Periodic Factory Inspections]
    I --> K[Label Authorization]
    J --> L{Compliance Check}
    L -->|Pass| M[Continued Listing]
    L -->|Fail| N[Corrective Action]
    N --> J
    M --> O[Annual Reassessment]
    O --> J
