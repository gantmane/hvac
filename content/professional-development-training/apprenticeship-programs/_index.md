...
```mermaid
graph TD
    A[Entry-Level Apprentice] --> B[First Year: Fundamentals]
    B --> C[Second Year: Systems]
    C --> D[Third Year: Advanced Installation]
    D --> E[Fourth Year: Diagnostics & Repair]
    E --> F[Fifth Year: Design & Leadership]
    F --> G[Journeyman Certification]
    G --> H[Master Technician Path]
    G --> I[Engineering Path]

    B --> B1[Safety & Tools]
    B --> B2[Basic Refrigeration]
    B --> B3[Electrical Fundamentals]

    C --> C1[Load Calculations]
    C --> C2[Duct Design]
    C --> C3[Piping Systems]

    D --> D1[Commercial Systems]
    D --> D2[Controls Integration]
    D --> D3[Energy Management]
```
