...
```mermaid
graph TD
    A[HVAC Retrofit Project] --> B[Federal Tax Credits]
    A --> C[State/Provincial Rebates]
    A --> D[Utility Incentives]
    A --> E[Local Programs]

    B --> F[Section 179D Deduction]
    B --> G[Investment Tax Credit]

    C --> H[Energy Office Rebates]
    C --> I[Sales Tax Exemption]

    D --> J[Efficiency Rebates]
    D --> K[Demand Response Payments]
    D --> L[Custom Incentives]

    F --> M[Total Project Economics]
    G --> M
    H --> M
    I --> M
    J --> M
