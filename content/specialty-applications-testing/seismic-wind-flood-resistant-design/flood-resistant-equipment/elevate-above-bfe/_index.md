...
```mermaid
graph TD
    A[Site Analysis] --> B{Located in SFHA?}
    B -->|Yes| C[Determine BFE from FIRM]
    B -->|No| D[Standard Installation]
    C --> E[Check Local Freeboard Requirements]
    E --> F[Calculate Required Elevation]
    F --> G{Equipment Type?}
    G -->|Critical| H[BFE + 2-3 ft minimum]
    G -->|Standard| I[BFE + 1-2 ft minimum]
    H --> J[Design Support Structure]
    I --> J
    J --> K[Verify Load Capacity]
    K --> L[Install Elevated Equipment]
```
...
```mermaid
graph LR
    subgraph "Elevation Methods"
        A[Ground Level] -->|+8-10 ft| B[Platform Mount]
        A -->|+15-30 ft| C[Rooftop Install]
        A -->|+3-6 ft| D[Elevated Slab]
    end

    subgraph "BFE Compliance"
        B --> E{Meets BFE + Freeboard?}
        C --> E
        D --> E
        E -->|Yes| F[Compliant Installation]
...
```mermaid
flowchart TB
    A[Equipment Selection] --> B[Determine BFE from FIRM]
    B --> C[Add Required Freeboard]
    C --> D[Select Elevation Method]
    D --> E{Rooftop Feasible?}
    E -->|Yes| F[Verify Roof Capacity]
    E -->|No| G[Design Platform/Room]
    F --> H[Design Supports]
    G --> H
    H --> I[Calculate Combined Loads]
    I --> J[Size Structural Members]
    J --> K[Detail Connections]
    K --> L[Obtain Permits]
    L --> M[Install and Document]
    M --> N[Elevation Certificate]
```
