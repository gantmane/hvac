...
```mermaid
graph TD
    A[Project Estimate] --> B[Material Takeoff]
    A --> C[Labor Hours]
    A --> D[Subcontractors]
    B --> E[Equipment Cost]
    B --> F[Material Cost]
    ...
    D --> I[Controls]
    D --> J[Test & Balance]
    E --> K[Total Direct Cost]
    F --> K
    G --> K
    H --> K
    I --> K
    J --> K
    K --> L[Apply Overhead & Profit]
    L --> M[Bid Price]
```
...
```mermaid
graph LR
    A[Financial KPIs] --> B[Gross Profit Margin %]
    A --> C[Net Profit Margin %]
    A --> D[Revenue per Employee]
    E[Operational KPIs] --> F[Utilization Rate %]
    E --> G[Days in Receivables]
    E --> H[Job Cost Variance %]
    I[Quality KPIs] --> J[Callback Rate %]
    I --> K[Customer Satisfaction]
    I --> L[Safety Incident Rate]
```
...
```mermaid
graph TD
    A[Growth Strategy] --> B[Organic Growth]
    A --> C[Acquisition]
    B --> D[Geographic Expansion]
    B --> E[Service Offering Expansion]
    B --> F[Market Segment Penetration]
    C --> G[Competitor Acquisition]
    C --> H[Capability Acquisition]
    D --> I[Assess Market Demand]
    E --> J[Evaluate Resource Requirements]
    F --> K[Increase Market Share]
    G --> L[Customer Base Integration]
    H --> M[Technology/Talent Addition]
```
