...
```mermaid
graph TD
    A[Energy Policy and Conservation Act EPCA 1975] --> B[National Appliance Energy Conservation Act NAECA 1987]
    B --> C[DOE Rulemaking Process]
    C --> D[Test Procedures]
    C --> E[Efficiency Standards]
    C --> F[Compliance Certification]

    D --> G[Manufacturer Testing]
    E --> G
    G --> H[Equipment Rating]

    H --> I[Minimum Efficiency Requirements]
    H --> J[ENERGY STAR Voluntary Programs]

    I --> K[Market Transformation]
    J --> K
    F --> K

    K --> L[Continuous Improvement Cycle]
    L --> C
